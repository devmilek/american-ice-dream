...
/** Fallback wyłącznie w kodzie — Keystatic (`content/lokalizacje/`) ma pierwszeństwo. */
export const locationDetails: Record<string, LocationDetails> = {};

export const getLocationDetails = cache(
	async (slug: string): Promise<LocationDetails | undefined> => {
		const fromKs = await keystaticReader.collections.lokalizacje.read(slug);
		if (fromKs) {
			return fromKs as unknown as LocationDetails;
		}
		return locationDetails[slug];
	},
);

export const getAllLocationDetails = cache(
	async (): Promise<LocationDetails[]> => {
		const fromFiles = await keystaticReader.collections.lokalizacje.all();
		const fromKs = fromFiles.map(
			({ entry }) => entry as unknown as LocationDetails,
		);
		const slugs = new Set(fromKs.map((d) => d.slug));
		const fromCode = Object.values(locationDetails).filter(
			(d) => !slugs.has(d.slug),
		);
		return [...fromKs, ...fromCode];
	},
);
