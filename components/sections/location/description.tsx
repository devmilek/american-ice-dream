...
export function LocationDescription({ location, details }: Props) {
  return (
    <section
      id="o-lokalu"
    ...
    >
      <div className="container-page grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:gap-20">
        <div className="lg:sticky lg:top-28 lg:self-start">
          <Eyebrow>Nasza lodziarnia w Katowicach</Eyebrow>
          <h2
            id="description-title"
            className="mt-4 mb-5 font-display text-[clamp(1.9rem,3.4vw+0.8rem,3.25rem)] leading-[1.05] tracking-[-0.02em] text-navy"
          >
            American Ice Dream{" "}
            <ScriptAccent size="sm">{location.city}</ScriptAccent>
            <span className="block text-[0.62em] font-normal italic text-navy-soft">
              — Twoja słodka wyspa w Auchan
            </span>
          </h2>
...
function TransportIcon({ variant }: { variant: "car" | "bus" | "parking" | "access" }) {
  if (variant === "car")
    return (
      <svg
        viewBox="0 0 24 24"
