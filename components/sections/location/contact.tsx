...
export function LocationContact({ location, details }: Props) {
    ...
    >
      <div className="container-page">
        <div className="mb-12 max-w-2xl">
          ...
          >
            Dane kontaktowe i{" "}
            <ScriptAccent size="sm">mapa</ScriptAccent>
          </h2>
          <p className="max-w-xl text-[15.5px] text-ink-soft">
            Wszystko, czego potrzebujesz, żeby do nas trafić — adres,
            godziny, telefon, kod pocztowy i dokładna lokalizacja na mapie.
          </p>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="flex flex-col gap-5 rounded-[28px] border border-navy/8 bg-paper p-7 shadow-[0_8px_24px_rgba(15,45,92,0.05)]">
            <InfoRow
              icon={<Pin className="h-4 w-4 text-gold" />}
              label="Adres"
            >
              <address className="not-italic">
                <strong className="font-semibold text-navy">
                  American Ice Dream · {location.venue}
                </strong>
                <br />
                {location.address}
                <br />
          ...
          <div className="relative h-[420px] overflow-hidden rounded-[28px] border border-navy/14 bg-[#eee4d1] shadow-[0_30px_70px_-30px_rgba(15,45,92,0.28)] lg:h-auto lg:min-h-[520px]">
            <SingleLocationMap location={location} zoom={15} />
          </div>
        </div>
      </div>
    </section>
        ...
        {weekdays.map((day, i) => {
          ...
          return (
            <tr key={day}>
              <th
                scope="row"
                className="py-1 text-left font-medium text-ink-soft"
              >
                {day}
              </th>
              <td className="py-1 text-right font-medium text-navy tabular-nums">
                {isSunday ? location.hours.sunday : location.hours.weekdays}
              </td>
            </tr>
          );
        })}
